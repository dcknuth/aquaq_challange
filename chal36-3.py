...
def solve(solution_states):
    ...
    time, pushing options onto the solution_states stack'''
    g, i = solution_states.pop()
    answer_set = []
    g_left = g.copy()
    i_left = deepcopy(i)
    while len(g_left) > 0:
        ...
        for num in all_pairs.keys():
            if len(all_pairs[num]) == g_needed[num]:
                ...
                # If there is more than 1, non-identical way to
                #  source an 'i', we need to try all of them
                i_orig = deepcopy(i_left)
                #if i_remove(i_left, x) and i_remove(i_left, y):
                ...
                found_force = True
                break
                # otherwise make a solve state for all options
                # TODO
...
while len(ls) > 0:
    ...
    print("\nCurrent puzzle is:")
    print(f"g:{g}\ni:{i}")
    solution_states = [[g, i_left]]
    ans = solve(solution_states)
